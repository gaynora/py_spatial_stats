...
point_gdf = geopandas.read_file('points.shp') #input points to a geodataframe
poly_gdf = geopandas.read_file('polygons.shp') #input polygons to a geodataframe

join_left_df = geopandas.sjoin(point_gdf, poly_gdf, how='left', op='intersects') # find and join polygon attributes to points using spatial join
